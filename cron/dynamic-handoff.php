...
function sb_init() {
    ...
    if (sb_isset_num($dynamic_handoff['dynamic-handoff-time']) 
        ...
        $conversations = sb_get_unassigned_conversations($dynamic_handoff['dynamic-handoff-time']);

        $dynamic_handoff_count = $dynamic_handoff['dynamic-handoff-count'];
...
function sb_get_unassigned_conversations($minute) {
    $conversations = sb_db_get('SELECT sb_conversations.id, sb_conversations.user_id FROM sb_conversations inner join sb_messages ON sb_conversations.id = sb_messages.conversation_id WHERE sb_conversations.status_code IN(0, 1, 2) AND sb_conversations.source = "sm" AND sb_messages.id IN ( SELECT max(sb_messages.id) FROM sb_messages WHERE sb_messages.creation_time < (utc_timestamp() - INTERVAL '. $minute .' MINUTE) group by sb_messages.conversation_id) ORDER BY sb_conversations.id DESC', false);
