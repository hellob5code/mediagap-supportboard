...
(function ($) {
    ...
    */
	...
	sms: {
            ...
            check: function (conversation) {
                return conversation.get('source') == 'sm';
            },

            send: function (to, message, attachments = [], onSuccess = false) {
                SBF.ajax({
                    function: 'sms-send-message',
                    to: to,
                    message: message,
                    attachments: attachments
                }, (response) => {
    ...
    var SBProfile = {
        ...
        populate: function (user, profile_area) {
            ...
            if (profile_area.hasClass('sb-profile-list-conversation') && SBChat.conversation) {
                ...
                if (!SBF.null(source)) {
                    code += this.profileRow('conversation-source', source == 'fb' ? 'Facebook' : (source == 'wa' ? 'WhatsApp' : (source == 'sm' ? 'SMS' : (source == 'tm' ? 'Text message' : (source == 'ig' ? 'Instagram' : source)))), sb_('Source'));
                }
            }
    ...
    */
    ...
    $(document).ready(function () {
        ...
        $(header).on('click', '.sb-version', function () {
            ...
            SBF.ajax({
                ...
                let code = '';
                let names = { 'sb': 'Support Board', 'slack': 'Slack', 'dialogflow': 'Dialogflow', 'tickets': 'Tickets', 'woocommerce': 'Woocommerce', 'ump': 'Ultimate Membership Pro', 'perfex': 'Perfex', 'whmcs': 'WHMCS', 'aecommerce': 'Active eCommerce', 'messenger': 'Messenger', 'whatsapp': 'WhatsApp', 'sms': 'SMS', 'armember': 'ARMember' };
                let updates = false;
        ...
        */
            ...
            if (!is_busy && !SBConversations.is_search && scrollPagination(this, true) && pagination_count) {
                ...
                SBF.ajax({
                    ...
                    if (pagination_count) {
                        ...
                        scrollPagination(this, false, 600);
                    }
        ...
        $(document).on('SBMessageSent', function (e, response) {
            ...
            if (SBApps.sms.check(SBChat.conversation)) {
                SBApps.sms.send(SBApps.sms.activeUserPhone(), response['message'], response['attachments'], (response) => {
                    if ('ErrorCode' in response) {
                        dialog('Error. Message not sent to SMS. Error message: ' + response.errorMessage, 'info');
                    }
                });
            }
	    if (SB_ADMIN_SETTINGS['smart-reply']) {
	...
	// SMS
        $(settings_area).on('click', '#sms-twilio-btn .sb-btn', function (e) {
            dialog(SB_URL + '/apps/sms/post.php', 'info');
            return false;
            e.preventDefault();
        });
