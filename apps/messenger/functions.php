    ...
    if ($page) {
        ...
        if (!empty($attachments) && is_array($attachments)) {
            for ($y = 0; $y < count($attachments); $y++) {
                ...
                $response = sb_curl('https://graph.facebook.com/me/messages?access_token=' . $page['messenger-page-token'], ['messaging_type' => 'RESPONSE', 'recipient' => ['id' => $psid], 'message' => ['attachment' => ['type' => $attachment_type, 'payload' => ['url' => $attachment[1], 'is_reusable' => true]], 'metadata' => $metadata, 'text' => '']]);
            }
        }
        return $response;
    }
...
function sb_messenger_add_user($user_id, $token, $user_type = 'lead', $instagram = false) {
    $user_details = json_decode(sb_get('https://graph.facebook.com/' . $user_id . ($instagram ? '?fields=name,profile_pic' : '?fields=first_name,last_name') . '&access_token=' . $token), true);
    if (sb_is_error($user_details)) return $user_details;
    $profile_image = $instagram ? $user_details['profile_pic'] : sb_isset(sb_isset(json_decode(sb_get('https://graph.facebook.com/' . $user_id . '/picture?redirect=false&width=600&height=600&access_token=' . $token), true), 'data'), 'url');
    if ($profile_image) {
